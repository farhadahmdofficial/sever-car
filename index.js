  ...
  try {
    ...
    app.patch('/bookings/:id', loger, verifyToken, async (req, res) => {
      try {
        ...
        // শুধুমাত্র নির্দিষ্ট নোডের স্ট্যাটাস 'Canceled' আপডেট করা
        const updateDoc = { $set: { status: 'Canceled' } };
        const result = await bookingCollection.updateOne(query, updateDoc);

        // গাড়ির অ্যাভাইলেবিলিটি আবার পুনরায় 'Available' করে রি-স্টোর করা
...
//     // 📤 ৭. নির্দিষ্ট ইউজারের বুকিং করা লিস্ট পাওয়ার রুট (GET) [🎯 404 ফিক্সড]
//    // 🔄 ৮. বুকিং ক্যান্সেল করার সিকিউর রুট (PATCH)
// app.patch('/bookings/:id', loger, verifyToken, async (req, res) => {
//   try {
//     const { id } = req.params;

//     // 🛡️ আইডি ভ্যালিডেশন চেক (যাতে ভুল আইডিতে ক্র্যাশ বা সব আপডেট না হয়)
//     if (!id || id === 'undefined' || !ObjectId.isValid(id)) {
//       return res.status(400).json({ error: "Invalid or missing Booking ID matrix." });
//     }
...
//     // প্রথমে নিশ্চিত হোন এই বুকিংটি ডাটাবেজে আছে কিনা
//     const targetBooking = await bookingCollection.findOne(query);
//     if (!targetBooking) {
//       return res.status(404).json({ error: "Target booking node not found." });
//     }

//     // শুধুমাত্র নির্দিষ্ট আইডি-র স্ট্যাটাস 'Canceled' করা
//     const updateDoc = { $set: { status: 'Canceled' } };
//     const result = await bookingCollection.updateOne(query, updateDoc);

//     // গাড়ির অ্যাভাইলেবিলিটি আবার 'Available' করে দেওয়া
//     if (targetBooking.carId && ObjectId.isValid(targetBooking.carId)) {
//       await carCollection.updateOne(
//         { _id: new ObjectId(targetBooking.carId) },
//         { $set: { availabilityStatus: 'Available' } }
...
// // 📤 ৭. বুকিং লিস্ট পাওয়ার রুট (GET) - ক্যানসেল হওয়াগুলো ফিল্টার আউট করার লজিকসহ
// app.get('/my-bookings', loger, verifyToken, async (req, res) => {
//   try {
//     const email = req.query.email;
//     if (!email) {
...
//     // 🎯 পরিবর্তন এখানে: যেগুলোর স্ট্যাটাস 'Canceled' নয়, শুধু সেগুলোই পেজে দেখাবে (Reload দিলেও আসবে না)
//     const query = { 
//       userEmail: email,
//       status: { $ne: 'Canceled' } 
//     };
...
// // রান ফাংশন এক্সিকিউট করা
// run().catch(console.dir);

// // পাবলিক বেস টেস্ট রুট
// app.get('/', (req, res) => {
//   res.send('Hello World! This is server side for car app.');
// });

// // সার্ভার লিসেনিং পোর্ট প্রোটোকল
// app.listen(port, () => {
